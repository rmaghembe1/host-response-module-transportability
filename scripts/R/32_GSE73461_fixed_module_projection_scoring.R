...
# GSE73461 fixed-module projection scoring
...
out_dir <- "results/module_projection/GSE73461_fixed_module_projection"
fig_dir <- "results/figures/GSE73461_fixed_module_projection"
docs_dir <- "docs"
...
projection_samples <- rbind(primary_samples, control_samples, use.names = TRUE)
projection_samples <- projection_samples[base_sample_id %in% expr_cols]

if (nrow(primary_samples[base_sample_id %in% expr_cols]) == 0) {
...
message("Secondary control samples: ", nrow(projection_samples[projection_role == "secondary_control_context"]))

# Collapse probes to gene SYMBOL by selecting the probe with highest median expression across projection samples.
...
fwrite(coverage, file.path(out_dir, "GSE73461_fixed_module_projection_coverage.tsv"), sep = "\t")
fwrite(scores_long, file.path(out_dir, "GSE73461_fixed_module_scores_long.tsv"), sep = "\t")
fwrite(scores_wide, file.path(out_dir, "GSE73461_fixed_module_scores_wide.tsv"), sep = "\t")
fwrite(group_summary, file.path(out_dir, "GSE73461_fixed_module_score_group_summary.tsv"), sep = "\t")
fwrite(primary_test, file.path(out_dir, "GSE73461_fixed_module_primary_projection_tests.tsv"), sep = "\t")
...
ggsave(file.path(fig_dir, "GSE73461_fixed_module_scores_primary_groups.png"), p1, width = 10, height = 6, dpi = 300)
ggsave(file.path(fig_dir, "GSE73461_fixed_module_scores_primary_groups.pdf"), p1, width = 10, height = 6)
...
ggsave(file.path(fig_dir, "GSE73461_fixed_module_primary_median_differences.png"), p2, width = 8, height = 5, dpi = 300)
ggsave(file.path(fig_dir, "GSE73461_fixed_module_primary_median_differences.pdf"), p2, width = 8, height = 5)

report_file <- "docs/GSE73461_fixed_module_projection_scoring_report.md"
sink(report_file)
cat("# GSE73461 Fixed-Module Projection Scoring Report\n\n")
cat("- Generated:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("- Purpose: score locked GSE211567 discovery modules in locked GSE73461 external projection cohort.\n")
...
cat("- Secondary control/context samples:", nrow(projection_samples[projection_role == "secondary_control_context"]), "\n\n")
...
cat("## Generated files\n\n")
cat("- `results/module_projection/GSE73461_fixed_module_projection/GSE73461_fixed_module_projection_coverage.tsv`\n")
cat("- `results/module_projection/GSE73461_fixed_module_projection/GSE73461_fixed_module_scores_long.tsv`\n")
cat("- `results/module_projection/GSE73461_fixed_module_projection/GSE73461_fixed_module_scores_wide.tsv`\n")
cat("- `results/module_projection/GSE73461_fixed_module_projection/GSE73461_fixed_module_score_group_summary.tsv`\n")
cat("- `results/module_projection/GSE73461_fixed_module_projection/GSE73461_fixed_module_primary_projection_tests.tsv`\n")
cat("- `results/module_projection/GSE73461_fixed_module_projection/GSE73461_gene_probe_choice_for_projection.tsv`\n")
cat("- `results/figures/GSE73461_fixed_module_projection/GSE73461_fixed_module_scores_primary_groups.png/.pdf`\n")
cat("- `results/figures/GSE73461_fixed_module_projection/GSE73461_fixed_module_primary_median_differences.png/.pdf`\n")
sink()

writeLines(capture.output(sessionInfo()), "env/session_info/GSE73461_fixed_module_projection_scoring_sessionInfo.txt")
