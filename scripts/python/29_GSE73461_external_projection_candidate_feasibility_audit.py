...
"""
GSE73461 external projection candidate feasibility audit.

Purpose:
- Audit GSE73461 as a candidate formal external projection cohort.
...
RAW_DIR = Path("data/metadata_raw/GSE73461")
OUT_DIR = Path("results/external_projection_candidate_audit/GSE73461")
...
SOFT = RAW_DIR / "GSE73461_family.soft.gz"
MATRIX = RAW_DIR / "GSE73461_series_matrix.txt.gz"
SUPP_INDEX = RAW_DIR / "GSE73461_supplementary_file_index.html"
...
def main() -> None:
    ...
    soft_df.to_csv(HARM_DIR / "GSE73461_GEO_family_SOFT_sample_metadata_flattened.tsv", sep="\t", index=False)
    matrix_df.to_csv(HARM_DIR / "GSE73461_series_matrix_sample_metadata.tsv", sep="\t", index=False)
    supp_df.to_csv(OUT_DIR / "GSE73461_supplementary_file_index.tsv", sep="\t", index=False)
    file_audit.to_csv(OUT_DIR / "GSE73461_raw_file_audit.tsv", sep="\t", index=False)
    ...
    field_presence = pd.DataFrame({
        ...
        "example_values": ["; ".join(soft_df[c].dropna().astype(str).unique()[:8]) for c in soft_df.columns],
    })
    field_presence.to_csv(OUT_DIR / "GSE73461_metadata_field_presence.tsv", sep="\t", index=False)

    keyword_patterns = {
        "bacterial_clues": r"bacter|sepsis|pneumonia|strept|staph|e\.?\s*coli|klebsiella|meningoc|pseudomon|haemophilus|gram.?positive|gram.?negative",
        "viral_clues": r"viral|virus|influenza|rsv|adenovirus|rhinovirus|sars|covid|coronavirus|enterovirus|dengue|h1n1",
        ...
        "respiratory_or_febrile_clues": r"respiratory|fever|febrile|acute infection|illness|pneumonia",
        ...
        "discovery_or_validation_clues": r"discovery|validation|training|test|cohort",
    }
    ...
    clue_df.to_csv(OUT_DIR / "GSE73461_metadata_keyword_clue_summary.tsv", sep="\t", index=False)

    if not supp_df.empty:
        supp_df["lower"] = supp_df["supplementary_file"].str.lower()
        supp_type_rows = []
        for label, pat in {
            "matrix_or_expression": r"count|counts|matrix|expression|expr|normalized|normalised|norm|raw|dataset",
            "metadata": r"metadata|clinical|sample|phenotype|pheno|annotation",
            "raw_archive": r"raw\.tar|cel|idat|tar",
    ...
    supp_type_df.to_csv(OUT_DIR / "GSE73461_supplementary_file_type_hints.tsv", sep="\t", index=False)
    ...
    control_hits = int(clue_df.loc[clue_df["clue"] == "control_clues", "sample_rows_matching_in_SOFT"].iloc[0])
    ...
    decision_df = pd.DataFrame([{
        ...
        "control_metadata_clue_rows": control_hits,
        "blood_or_pbmc_clue_rows": blood_hits,
        "preliminary_status": preliminary_status,
        "next_action": "Inspect phenotype fields and processed expression matrix structure before any cohort-lock decision.",
    }])
    decision_df.to_csv(OUT_DIR / "GSE73461_preliminary_external_projection_candidate_decision.tsv", sep="\t", index=False)

    report = DOCS_DIR / "GSE73461_external_projection_candidate_feasibility_audit_report.md"
    report.write_text(
        "\n".join([
